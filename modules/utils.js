...
const updateBalances = (address, token) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const result = await this.osseus.db_models.wallet.updateBalances({'address': address, 'balances.currency': currency.id}, balance, balance)
      resolve(result)
    } catch (err) {
      reject(err)
    }
