...
contract('CURRENCY', async (accounts) => {
  ...
  const validateCurrency = (currency) => {
    ...
    expect(currency.ccAddress).to.equal(ccAddress)
    expect(currency.mmAddress).to.equal(mmAddress)
    expect(currency.ccABI).to.equal(ccABI)
    expect(currency.mmABI).to.equal(mmABI)
  }
  ...
  it('should get currency (by id)', async () => {
    let currency = await osseus.lib.Currency.create(ccAddress, mmAddress, ccABI, mmABI)
    currency = await osseus.db_models.currency.getById(currency.id)
    validateCurrency(currency)
  })

  it('should get currency (by cc address)', async () => {
    let currency = await osseus.lib.Currency.create(ccAddress, mmAddress, ccABI, mmABI)
    currency = await osseus.db_models.currency.getByCurrencyAddress(currency.ccAddress)
    validateCurrency(currency)
  })

  it('should get currency (by mm address)', async () => {
    let currency = await osseus.lib.Currency.create(ccAddress, mmAddress, ccABI, mmABI)
    currency = await osseus.db_models.currency.getByMarketMakerAddress(currency.mmAddress)
    validateCurrency(currency)
  })
  ...
  it('should get error if currency not found (by id)', async () => {
    let fakeId = '123abc'
    let currency = await osseus.lib.Currency.create(ccAddress, mmAddress, ccABI, mmABI)
    currency = await osseus.db_models.currency.getById(fakeId).catch(err => {
      expect(err).not.to.be.undefined
    })
    expect(currency).to.be.undefined
  })
